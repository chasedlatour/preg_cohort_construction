...
describe_analytic <- describe %>% 
  mutate(
    ...
    n_rural = as.double(str_extract(n_rural, "^\\d+")),
...
# This function takes a value stored as (e.g.) 5000 (5%) and prints it as 5,000 (5%) -- it introduces the correctly spaced commas.
format_numbers <- function(x) {
  
  num <- as.double(str_extract(x, "^\\d+"))  # Extract the numeric part before the parentheses
  percent <- str_extract(x, "(?<=\\()\\d+%(?=\\))")  # Extract the percentage inside the parentheses
  
  num_format <- format(num, big.mark = ",")  # Add commas to the number part
  
  # Combine the formatted number and the percentage
  final <- paste0(num_format, " (", percent, ")")  # Format it as "5,000 (5%)"
...
describe_cohorts <- function(describe_subset) {
  ...
  hold <- describe_subset %>% 
    mutate(
      missing_mar = format_numbers(missing_mar),
      missing_mnar = format_numbers(missing_mnar),
      across(c("n_low", "n_med", "n_high", "n_rural", "n_miscarriage_all",
                    "n_stillbirth_all", "n_livebirth_all"),
                  ~ format_numbers(.))
    ) %>%
    select(marginal_p_miss_severity, marginal_p_miss_miscarriage,
           missing_mar, missing_mnar, trt,
           n_low, n_med, n_high, n_rural,
    ...
    collapse_rows(col_names = c("pnc_wk", "marginal_p_miss_severity", "marginal_p_miss_miscarriage"),
                  valign = "top") %>%
    kable_classic("striped")  # Adds stripes for better readability
...
describe_pregnancies <- function(describe_subset) {
  ...
  hold <- describe_subset %>% 
    mutate(
      marginal_p_miscarriage = marginal_p_miss_miscarriage / 4.9,
      n_total = format(n_total, big.mark = ","),
      n_preec = format(n_preec, big.mark = ","),
      n_pregnancies_deliveries = format(n_pregnancies_deliveries, big.mark = ","),
      n_preeclampsia_deliveries = format(n_preeclampsia_deliveries, big.mark = ","),
      n_pregnancies_outcomes = format(n_pregnancies_outcomes, big.mark = ","),
      n_preeclampsia_outcomes = format(n_preeclampsia_outcomes, big.mark = ","),
      n_pregnancies_all = format(n_pregnancies_all, big.mark = ","),
      n_preeclampsia_all = format(n_preeclampsia_all, big.mark = ",")
    ) %>% 
    select(marginal_p_miss_severity, marginal_p_miss_miscarriage,
           trt, n_total, n_preec, n_pregnancies_deliveries, n_preeclampsia_deliveries, 
           n_pregnancies_outcomes, n_preeclampsia_outcomes, n_pregnancies_all, 
           n_preeclampsia_all) %>% 
    kbl() %>%
...
analyses2 <- analyses %>% 
  ...
  mutate(
    obs_del_risk0_bias = 100*(obs_del_risk0 - truth_risk0),
    obs_del_risk1_bias = 100*(obs_del_risk1 - truth_risk1),
    obs_del_rd_bias = 100*(obs_del_rd - truth_rd),
    obs_del_rr_bias = log(obs_del_rr) - log(truth_rr),
    obs_del_or_bias = log(obs_del_or) - log(truth_or),
    obs_out_risk0_bias = 100*(obs_out_risk0 - truth_risk0),
    obs_out_risk1_bias = 100*(obs_out_risk1 - truth_risk1),
    obs_out_rd_bias = 100*(obs_out_rd - truth_rd),
    obs_out_rr_bias = log(obs_out_rr) - log(truth_rr),
    obs_out_or_bias = log(obs_out_or) - log(truth_or),
    all_preg_risk0_bias = 100*(all_preg_risk0 - truth_risk0),
    all_preg_risk1_bias = 100*(all_preg_risk1 - truth_risk1),
    all_preg_rd_bias = 100*(all_preg_rd - truth_rd),
    ...
    all_preg_all_outc_risk0_bias = 100*(all_preg_all_outc_risk0 - truth_risk0),
    all_preg_all_outc_risk1_bias = 100*(all_preg_all_outc_risk1 - truth_risk1),
    all_preg_all_outc_rd_bias = 100*(all_preg_all_outc_rd - truth_rd),
    all_preg_all_outc_rr_bias = log(all_preg_all_outc_rr) - log(truth_rr),
    
    trt_preg_outc_risk0_bias = 100*(trt_preg_outc_risk0 - truth_risk0),
    trt_preg_outc_risk1_bias = 100*(trt_preg_outc_risk1 - truth_risk1),
    trt_preg_outc_rd_bias = 100*(trt_preg_outc_rd - truth_rd),
    trt_preg_outc_rr_bias = log(trt_preg_outc_rr) - log(truth_rr),
    
    untrt_preg_outc_risk0_bias = 100*(untrt_preg_outc_risk0 - truth_risk0),
    untrt_preg_outc_risk1_bias = 100*(untrt_preg_outc_risk1 - truth_risk1),
    untrt_preg_outc_rd_bias = 100*(untrt_preg_outc_rd - truth_rd),
    untrt_preg_outc_rr_bias = log(untrt_preg_outc_rr) - log(truth_rr),

    all_preg_no_outc_risk0_bias = 100*(all_preg_no_outc_risk0 - truth_risk0),
    all_preg_no_outc_risk1_bias = 100*(all_preg_no_outc_risk1 - truth_risk1),
    all_preg_no_outc_rd_bias = 100*(all_preg_no_outc_rd - truth_rd),
    all_preg_no_outc_rr_bias = log(all_preg_no_outc_rr) - log(truth_rr),
    
    # Make all the RDs as per 100 pregnancies
    truth_rd = 100*truth_rd,
    obs_del_rd = 100*obs_del_rd,
    obs_out_rd = 100*obs_out_rd,
    all_preg_rd = 100*all_preg_rd,
    all_preg_all_outc_rd = 100*all_preg_all_outc_rd,
    trt_preg_outc_rd = 100*trt_preg_outc_rd,
    untrt_preg_outc_rd = 100*untrt_preg_outc_rd,
    all_preg_no_outc_rd = 100*all_preg_no_outc_rd
  ) %>% 
...
intermediate <- describe %>% 
  mutate(
    ...
    n_high_num = as.double(str_extract(n_high, "^\\d+")),
    n_total = n_low_num + n_med_num + n_high_num,
    n_rural_num = as.double(str_extract(n_rural, "^\\d+")),
...
create_biasfigure_nosens <- function(dataset, parameter, rr_preec, title, y_min, y_max, parameter_yaxis = "Risk Difference (per 100)"){
  
  param <- sym(parameter)
  # rr_abort <- sym(rr_abort)
  rr_preeclampsia <- sym(rr_preec)
  
  wong_colors <- c("#000000", "#E69F00", "#56B4E9")
  ...
  plot <- dataset %>%
    ...
    facet_wrap(missing ~., nrow = 1, 
               labeller = labeller(missing = 
                                     c("0/0.05" = "All MNAR: 0% Severity, 100% Miscarriage",
                                       "0.025/0.025" = "MAR+MNAR: 50% Severity, 50% Miscarriage",
                                       "0.05/0" = " All MNAR: 100% Severity, 0% Miscarriage",
                                       "0/0.2" = "All MNAR: 0% Severity, 100% Miscarriage",
                                       "0.1/0.1" = "MAR+MNAR: 50% Severity, 50% Miscarriage",
                                       "0.2/0" = "All MAR: 100% Severity, 0% Miscarriage"))) +
    geom_hline(aes(yintercept = 0), col = "black", linetype = "dashed") +
    xlab("Effect of Initiation on the Risk of Miscarriage") + 
    ylab(paste0("Bias in the\n", parameter_yaxis)) +
    ggtitle(title) +
...
create_biasfigure_sens <- function(dataset, parameter, rr_preec, title, y_min, 
                                   y_max, parameter_yaxis="Risk Difference"){
  
  param <- sym(parameter)
  # rr_abort <- sym(rr_abort)
  rr_preeclampsia <- sym(rr_preec)
  
  wong_colors <- c("#000000", "#E69F00", "#56B4E9", "#009E73", "#0072B2","#D55E00","#CC79A7")
...
create_biasfigure_nosens(bias_figure_data2_5pct, "Risk Difference", "0.5",
                     "(C) Risk Difference", y_min=-7.5, y_max=7.5,
...
pdf("Bias Risk ratio by Missingness - No effect risk of Preeclampsia - No sensitivity analyses - 5pct.pdf", height = 2, width = 10)
create_biasfigure_nosens(bias_figure_data2_5pct, "Risk Ratio", "1", 
                     "(D) Risk Ratio",
                     -0.2, 0.4,
                     "Risk Ratio (Log-Scaled)")
dev.off()
