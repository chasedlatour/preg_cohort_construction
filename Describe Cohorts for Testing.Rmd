...
test1 <- do.call(rbind, readRDS("ab08preec08_beta001_gamma01_001.rds")) %>% 
...
describe_cohort <- function(dataset){
  ...
  hold <- dataset %>% 
    ...
    pack_rows("Number (%) of Pregnancies that Experienced Preeclampsia", 13, 13) %>% 
    pack_rows("Median(IQR) of Preeclampsia Timing", 14, 14) %>% 
    pack_rows("Number (%) of Pregnancy Outcomes LTFU per MAR", 15, 15) %>% 
    pack_rows("Median (IQR) of LTFU Timing per MAR", 16, 16) %>% 
    pack_rows("Number (%) of Pregnancies LTFU per MAR+MNAR", 17, 17) %>% 
    pack_rows("Median (IQR) of LTFU Timing per MAR+MNAR", 18, 18)
