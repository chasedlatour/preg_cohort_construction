...
describe_cohort <- function(dataset){
  ...
  hold <- dataset %>% 
    ...
    pack_rows("Number (%) of Pregnancy Outcomes LTFU per MAR", 14, 14) %>% 
    pack_rows("Median (IQR) of LTFU Timing per MAR", 15, 15) %>% 
    pack_rows("Number (%) of Pregnancies LTFU per MAR+MNAR", 16, 16) %>% 
    pack_rows("Median (IQR) of LTFU Timing per MAR+MNAR", 17, 17)
